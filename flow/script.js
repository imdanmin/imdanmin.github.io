...
function updateButtonStates() {
  ...
  if (isRunning) {
    ...
    resetBtn.textContent =
      !isWorkMode || pausedMode === "break" ? "Skip Break" : "Reset";
  }
}
...
function saveSessions() {
  ...
  document.querySelectorAll("#session-log .session-entry").forEach((el) => {
    ...
    const time = el.querySelector(".session-time")
...
function toggleTaskList() {
  ...
  localStorage.setItem(
    "flowmodoro_tasks_visible",
    JSON.stringify(isTasksVisible)
  );
}
...
function updateModeClasses() {
  ...
  sessionLogSection.classList.remove(
    "standby-mode",
    "work-mode",
    "break-mode",
    "paused-work",
  ...
  particleSpawner = setInterval(() => {
    ...
    if (currentParticleType === "snow") {
      ...
      particle.classList.add(
        driftTypes[Math.floor(Math.random() * driftTypes.length)]
      );
